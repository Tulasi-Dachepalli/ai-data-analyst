...
import EmailDigestDispatcher from "./EmailDigestDispatcher";
import GoalSeekSolver from "./GoalSeekSolver";
import RfmSegmentation from "./RfmSegmentation";
import GeoMapVisualizer from "./GeoMapVisualizer";
...
export default function DataAnalystDashboardBot({ currentView, user: propUser }) {
  ...
  const handleDownloadExecutivePdf = (thread) => {
    try {
      if (!thread || !thread.dashboard) return;
      const safeRows = thread.rows || [];
      const safeCols = thread.columns || [];
      const safeStats = thread.stats || [];
      const generatedDate = new Date().toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" });
      const datasetName = thread.fileName || thread.name || "Executive Workspace Dataset";

      const domainKpis = computeDomainPresetKpis(safeRows, safeCols, thread.dashboard?.plan || {});
      const qualityScore = thread.quality ? thread.quality.score : 100;
      const missingCells = thread.quality ? thread.quality.missingCells : 0;

      const kpiHtml = domainKpis.map(k => `
        <div style="flex:1;min-width:160px;background:#F8FAFC;border:1px solid #E2E8F0;border-radius:10px;padding:16px;text-align:center;">
          <div style="font-size:22px;font-weight:800;color:#0F172A;margin-bottom:4px;">${k.value}</div>
      ...
      const pdfHtml = `
        ...
        <head>
          <meta charset="utf-8" />
          <title>${datasetName} — Executive Analytics Deck</title>
          <style>
            @page { size: A4 portrait; margin: 15mm; }
            body { font-family: 'Segoe UI', system-ui, -apple-system, sans-serif; background: #FFF; color: #0F172A; margin: 0; padding: 20px; line-height: 1.5; }
            ...
            .badge { background: #0F172A; color: #FFF; padding: 4px 10px; border-radius: 4px; font-size: 11px; font-weight: 700; text-transform: uppercase; }
            .kpi-container { display: flex; flex-wrap: wrap; gap: 12px; margin-bottom: 24px; }
            .section { margin-bottom: 24px; background: #FAFAFA; border: 1px solid #F1F5F9; border-radius: 10px; padding: 20px; }
            .section-title { font-size: 15px; font-weight: 700; color: #0F172A; margin-bottom: 10px; text-transform: uppercase; letter-spacing: 0.5px; border-left: 4px solid #8B5CF6; padding-left: 10px; }
            .footer { border-top: 1px solid #E2E8F0; padding-top: 14px; font-size: 11px; color: #94A3B8; display: flex; justify-content: space-between; margin-top: 30px; }
        ...
        <body>
          <div class="header">
            <div>
              <span class="badge">Executive Board Deck</span>
              <h1 style="margin:8px 0 4px 0;font-size:24px;color:#0F172A;">${datasetName}</h1>
              <div style="font-size:12px;color:#64748B;">Generated: ${generatedDate} • ${safeRows.length.toLocaleString()} Rows × ${safeCols.length} Columns</div>
            </div>
            <div style="text-align:right;">
              <div style="font-size:16px;font-weight:800;color:#8B5CF6;">AI Data Analyst</div>
              <div style="font-size:11px;color:#64748B;">Executive Enterprise Deck</div>
            </div>
          </div>

          <div class="section">
            <div class="section-title">🏆 Data Quality Certification & Key Metrics</div>
            <div class="kpi-container">
              <div style="flex:1;min-width:160px;background:#ECFDF5;border:1px solid #A7F3D0;border-radius:10px;padding:16px;text-align:center;">
                <div style="font-size:22px;font-weight:800;color:#059669;">${qualityScore}%</div>
                <div style="font-size:11px;font-weight:700;color:#047857;text-transform:uppercase;">Data Quality Score</div>
                <div style="font-size:10px;color:#065F46;margin-top:2px;">${missingCells} missing cells</div>
              </div>
              ${kpiHtml}
            </div>
          </div>
          ...
          <div class="section">
            <div class="section-title">📊 Executive Summary & Insights</div>
            <div style="font-size:13.5px;color:#334155;line-height:1.7;white-space:pre-wrap;">${thread.dashboard?.narrative || "Comprehensive automated executive summary generated by AI Data Copilot."}</div>
          </div>

          <div class="footer">
            <span>AI Data Analyst & Science Platform • Executive Edition</span>
      ...
      if (printWin) {
        ...
        alert("Please allow popups to open the Executive PDF Deck.");
      }
    } catch (err) {
      alert("Failed to generate Executive PDF: " + err.message);
      console.error(err);
    }
  };
